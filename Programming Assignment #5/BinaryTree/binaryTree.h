...
public:
    ...
    //returns the number of nodes with only one child
...
private:
    ...
    //tree to which p points 
    int single(binaryTreeNode<elemType>* p) const;
    //returns the number of nodes with only one child
};
...
//recursive function to find the number of nodes with only one child
template<class elemType>
int binaryTreeType<elemType>::single(binaryTreeNode<elemType>* p) const
{
    if (p != NULL) //makes sure that the root node is not null
    {
        //if the current node has either only a right link or a left link
        if ((p->llink == NULL && p->rlink != NULL) || (p->rlink == NULL && p->llink != NULL))
        {
            return 1 + single(p->llink) + single(p->rlink); //add 1 and go to the left subtree and then the right subtree
        }
        else
            return 0 + single(p->llink) + single(p->rlink); //add 0 and go to the left subtree and then the right subtree
    }
    else 
        return 0; //else, the current node is null so return 0
}
